...
PNK.set = function (el)
{
    ...
    this.table = this.el.getAttribute('pnk-table')
	return this;
}
...
PNK.create = function (tableid)
{
  ...
  let div = this.el;
	...
	if (typeof pnk_src[pnksrc] != 'undefined') {
		...
		$.getJSON( "cm/describe?t="+div.getAttribute('pnk-table'), function( data ) {
			pnk_src[pnksrc] = data;
			PNK.create_table(tableid, data);
		});
	}
...
PNK.create_table = function (tableid, data)
{
  var div=$(this.el)
	...
	if(isdef(data.grouping)){
			gsr+='<div>'+_e('Group by')+'<select class="pnk-groupby g-input"><option value=""></option>';
  ...
  if(isdef(data['search-box']))
    gsr=gsr+'<div col="search"><input type="text" class="pnk-input g-input" onkeypress="if(event.keyCode || event.which == \'13\') $(\'.com-search\').click()" value=""></div>';
	...
	if(isdef(data['search-boxes'])) for(i=0;i<data['search-boxes'].length;i++){
		...
		gsr=gsr+'<div col="'+fid+'"><label>'+ftitle+'</label> '+PNK.createFilter(df, ['','',''] )+'</div>';
		// We add the datalists in the end of the function
	}

	// Buttons //'+_e('Search')+'
	if(gsr!="") gsr='<div class="pnk-searchbox" style="display:inline-block">'+gsr+'<button class="g-btn btn-default com-search"><i class="fa fa-search"></i> </button></div>';
...
function pnk_table_head(div, tableid, data){
  var tls = '';
    ...
    if(PNK.tools[tl]) {
      ...
      tls+=' <button type="button" class="g-btn btn-default" tool="'+tl+'" '+bc+xtr+'>'+tfa+" "+tb+aftr+'</button>';
    }
	}
...
PNK.load = function ()
{
	...
	var groupby=$(table).attr('group-by');
	...
	$.getJSON( "cm/list_rows/"+this.table+"?page="+page+porderby+pgroupby+pgroupfor+filters, function( data ) {
		...
		if(isdef(data['rows'])){
            ...
            if(typeof $.tooltip != 'undefined') $('.pnk-com').tooltip();
		}else tbody.html('<tr><td colspan="'+PNK.colspan+'">No results found</td></tr>');
...
PNK.cv = function (cv,fc,rv)
{
	...
	if(cv!=null) {
		...
		if(isdef(fc.type)) if(isdef(PNK.field_types[fc.type]))
				if(isdef(PNK.field_types[fc.type].cv)) {
					return PNK.field_types[fc.type].cv(fc,cv)
				}
...
PNK.tfoot = function (data)
{
	...
	if(isdef(pagination)) if($('table',this.el).attr('group-by')==''){
		...
		return '<tr><td colspan="'+this.colspan+'"><ul class="pagination pnk-pagination">'+ PNK.pagination(parseInt(curPage), parseInt(totalPage)) +'</ul></td></tr>'
...
$(document).on('click','.pnk-table tr .col-ch p',function()
{
	...
	var t=$(this).closest('.pnk-table').attr('pnk-src');
	var tname=$(this).closest('.pnk-table').attr('pnk-table');
	//var tid=$(this).closest('table').attr('id');
	var col=cell.attr('col');
	var v=cell.attr('value');
	if(v==0) v=1; else v=0;
	$.post(
		"cm/update_rows?t="+tname+"&id="+tr.attr('row-id'),
		{ [col]: v },
		 function(data) {
...
$(document).on('click','.pnk-table .up-button',function()
{
	...
	$.ajax({
		url: "cm/update_rows?t="+t+"&id="+tr.attr('row-id'),
		data: { [col] : v },
		dataType: "json",
		type: 'post',
...
function pnk_delete_row(tr) {
  ...
  var tname = tr.closest('.pnk-table').attr('pnk-table');
  $.ajax({
		url: "cm/delete?t="+tname,
		data: { 'id' : id },
		type: 'post',
		success: function(data) {
...
function edit_dialog(pnk_table,action,id=""){
	...
	var tname=$(pnk_table).closest('.pnk-table').attr('pnk-table');
  ...
  var row_id="";
	if(id==""){
		dbupt=_e('Save');
		dbt=_e('New Registry');
		ajax_url = "cm/empty_row?t="+tname;
	}else{
		dbupt=_e('Update');
		dbt=_e('Edit Registry');
		ajax_url = "cm/list_rows?t="+tname+"&id="+id;
    	row_id=id;
	}
  	if(action=="clone"){
    	dbupt=_e('Save');
	...
	$.ajax({
		url: ajax_url, dataType: "json",type: 'post',
		success: function(data) {
			...
			for(i=0; i<data.fields.length; i++) {
				...
				sopt=sopt+'<div class="update-div" col="'+data.fields[i]+'"><label class="g-label" for="'+data.fields[i]+'">'+title+'</label>'+PNK.createInput(field,data.rows[0][i])+'</div>';
			}

			//<span class="pnk-edit-title">'+dbt+'</span>
			editDiv.html('<h3 class="pnk-edit-title">'+dbt+'</h3><div style="width: 100%;display: inline-block;margin-bottom:8px;border-bottom:1px solid #ccc"></div>'+sopt+btns);
		   ...
		   if(typeof(CodeMirror) != "undefined") if(cm_dom = g('.codemirror-js').all[0])
			    pnk_cmirror=CodeMirror.fromTextArea(cm_dom, {lineNumbers:true,mode:'javascript'});
		}
	});
}
...
PNK.edit_save = function (btn)
{
	...
	$(btn).closest('.pnk-edit').find( '.update-div' ).each( function() {
		...
		if(_input.prop("tagName")=='TEXTAREA') {
			if(_input.hasClass('codemirror-js') && (typeof pnk_cmirror!='undefined')) {
				/*try {
	...
	data =JSON.parse(data);
		...
		$.ajax({
			url: "cm/update_rows?t="+tname+"&id="+id, data: data, dataType: "json", type: 'post',
			success: function(data) {
				if(id!=""){
...
PNK.createInput = function (df,v)
{
	...
	if(df['options']) if(df['type']!="roles"){
    	...
    	if(df['type']=='joins' || df['type']=='meta') return '<select multiple class="g-input pnk-select" '+d_l_s+'>'+sopt+'</select>';
    	return '<select class="g-input pnk-select" '+d_l_s+'>'+sopt+'</select>';
	}


  	// Field Types
	switch(df.type) {
		case 'checkbox': return '<input type="checkbox" class="pnk-input g-input" value="'+(v==1?'1':'0')+'" '+(v==1?'checked':'')+'>';
		case 'text': return '<textarea class="g-input codemirror-js">'+v+'</textarea>';
		case 'date': return '<input class=" g-input datepicker" value="'+v+'">';
		case 'password': return '<input class=" g-input" type="password" value="'+v+'">';
		case 'datetime': return '<input class="g-input datetimepicker" value="'+v+'">';
	}

	if(isdef(PNK.field_types[df.type])) if(isdef(PNK.field_types[df.type].displayInput))
		return PNK.field_types[df.type].displayInput(df,v);

	return '<input type="text" class="g-input" value="'+v+'">';
}
...
PNK.createFilter = function (df,v){
	...
	return '<input type="text" colf="[has]" class="pnk-input '+typec+'" value="'+v[0]+'">';
}
...
PNK.tools = {
  ...
  pdf: { fa: "file-text", title: "Pdf", fn: function(e){ PNK.window_open(e.table,"pdf"); }},
...
PNK.window_open = function (e,file) {
	var pt = e.pnk_table;
	var t = pt.attr('pnk-src');
	var orderby = pt.attr('order-by');
	var groupby = pt.attr('group-by');
	var filters = pt.attr('filters');
	if(file=='csv') {
		window.open("cm/csv?t="+pt.attr('pnk-table')+"&file="+file+"&orderby="+orderby+"&groupby="+groupby+filters);
	}
}
...
$(document).on("click",".col-group",function(){
  ...
  if(group_level == '') group_level = 0;
  	...
  	// Get the rows from the source file
	$.getJSON( "list_rows/"+t+"?page=1"+groupby+filters, function( data ) {
