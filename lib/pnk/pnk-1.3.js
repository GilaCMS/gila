...
PNK.create_table = function (tableid, data)
{
  ...
  if(typeof data['search-box'] !== 'undefined')
	...
	if(typeof data['search-boxes'] !== 'undefined') for(i=0;i<data['search-boxes'].length;i++){
		...
		if(typeof(df.title) !='undefined') ftitle=df.title; else ftitle=fid;
	...
	if(typeof rows!='undefined') for(var i = 0, len = rows.length; i < len; i++) {
    	...
    	for(var j = 0, lenj = rows[i].length; j < lenj; j++) {
			...
			tdclass=''
      		...
      		if(field.type=='number') tdclass+=' col-number';
			if(field.type=='gallery') tdclass+=' col-number';
			if(field.type=='roles') tdclass+=' col-roles';
			if(field.type=='checkbox') tdclass+=' col-ch';
...
PNK.thead = function (id,data)
{
  ...
  if(typeof pnk_srcid['bulk_actions']!="undefined"){ // Add checkbox
...
PNK.cv = function (cv,fc,rv)
{
	...
	/////// Eval
	if(typeof fc.eval !== "undefined" ) eval(fc.eval);
...
PNK.tfoot = function (data)
{
	...
	if(typeof pagination!== "undefined") if($('table',this.el).attr('group-by')==''){
...
$(document).on('click','.pnk-table table tbody .main-row td',function(){
	...
	if(typeof pnk_src[pid].td_command!='undefined') $(this).closest('.pnk-table').find('.com-'+pnk_src[pid].td_command).click();
...
PNK.edit_save = function (btn)
{
	...
	if(typeof filters!="undefined") if(id==''){
		...
		for (i = 0; i < farray.length; i++) if(typeof pnk_src[PNK.src].fields[i]!='undefined') {
			frow=farray[i].split('=');
			if(dcv!='') dcv+=',';
			dcv+='"'+frow[0]+'":"'+frow[1]+'"';
		}
...
PNK.createInput = function (df,v)
{
	...
	switch(df.type) {
		...
		//case 'json': return '<textarea class="pnk-input codemirror-js">'+v+'</textarea>';
	}
