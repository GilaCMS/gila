	...
	$(this).closest('.pnk-edit').find( '.update-div' ).each( function() {
		...
		if(_input.prop("tagName")=='TEXTAREA') {
			if(_input.hasClass('codemirror-js') && (typeof pnk_cmirror!='undefined')) {
				...
				dcv+='"'+$(this).attr('col')+'":'+JSON.stringify(pnk_cmirror.getValue());
				delete pnk_cmirror;
			} else dcv+='"'+$(this).attr('col')+'":"'+_input.html()+'"';
		}else dcv+='"'+$(this).attr('col')+'":"'+_input.val()+'"';
	});
...
function createInput(df,v){
  ...
  // Textarea
	if(df['type']=='text') return '<textarea class="pnk-input codemirror-js">'+v+'</textarea>';
	// Date
	...
	if(df['type']=='json') return '<input class="pnk-input codemirror-js" value="'+v+'">';
