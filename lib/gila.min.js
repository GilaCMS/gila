...
g.dialog = function (p){
    var default_params = {class:'',escape:true,body:'',title:'',foot:'',img:'',buttons:'ok',type:'',id:'gila-popup',zindex:0};
    ...
    if(p.zindex >0) {
      style=' style="z-index:'+p.zindex+'"';
    }
    // onclick="event.stopPropagation()"
    append='<div id=\''+p.id+'\' class="'+p.class+' gila-popup">'+closebtn+p.title+p.img+p.body+p.foot+'</div>';
    if(p.type=='modal'){
      g(document.body).append('<div class=\'gila-darkscreen\''+dsclick+style+'">'+append+'</div>');
