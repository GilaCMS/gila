...
g.ajax = function(path,params,fn,type){
    var xhttp = new XMLHttpRequest();
        ...
        xhttp.callback = function() {
          if (this.readyState == 4 && this.status == 200) {
              if(typeof type!='undefined') {
                  if(type=='json') this.responseText = JSON.parse(this.responseText);
              }
...
g.post = function(path,params,fn){
    ...
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); //multipart/form-data
    xhttp.send(params);
}
g.upload = function(path,params,fn){
    var xhttp = new XMLHttpRequest();
...
g.dialog = function (p){
    ...
    g(document.body).append('<div id=\'gila-darkscreen\''+dsclick+'"><div id=\'gila-popup\' class="'+p.class+'">'+closebtn+p.title+p.img+p.body+p.foot+'</div></div>');
}
