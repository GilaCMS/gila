        ...
        <div class="carousel-item <?= $i==0?'active':'' ?>">
          ...
          <div class="carousel-caption d-none d-md-block <?=( isset($data['text-align'])?'text-'.$data['text-align']:'') ?>"  style="height: <?=( isset($data['vertical-align'])?(($data['vertical-align'])*10).'%':'') ?>">
            <?php if (isset($page['title'])) { ?>
              <h5 style="<?= isset($data['color-text'])?'color:'.$data['color-text']:'color:black' ?>;font-size:<?= (($data['title-font-size'])?$data['title-font-size'].'px':'40px') ?>" ><?= $page['title'] ?></h5>
            <?php } ?>
            <?php if (isset($page['description'])) { ?>
              <p style="<?= isset($data['color-text'])?'color:'.$data['color-text']:'color:black' ?>;font-size:<?= (($data['description-font-size'])?$data['description-font-size'].'px':'25px') ?>"><?= $page['description'] ?></p>
            <?php } ?>
