...
<div id="MyCarousel" class="carousel slide carousel-dark <?= ($data['carousel-full-width']=='1')?'position-absolute w-100 start-0':'' ?>" data-bs-ride="carousel">
        ...
        <div class="carousel-item <?= $i==0?'active':'' ?>">
          ...
          <div class="carousel-caption d-none d-md-block <?=( isset($data['text-align'])?'text-'.$data['text-align']:'') ?>"  style="height: <?=( isset($data['vertical-align'])?(($data['vertical-align'])*10).'%':'') ?>">
            <?php if (isset($post['title'])) { ?>
              <h5 style="<?= isset($data['color-text'])?'color:'.$data['color-text']:'color:black' ?>;font-size:<?= (($data['title-font-size'])?$data['title-font-size'].'px':'40px') ?>" ><?= $post['title'] ?></h5>
            <?php } ?>
            <?php if (isset($post['description'])) { ?>
              <p style="<?= isset($data['color-text'])?'color:'.$data['color-text']:'color:black' ?>;font-size:<?= (($data['description-font-size'])?$data['description-font-size'].'px':'25px') ?>"><?= $post['description'] ?></p>
            <?php } ?>
