...
var g_swipe_queries = {up:[], down:[], left:[], right:[]};
g.el = function(id) {
...
g.swipe = function (query, direction, fn) {
  g_swipe_queries[direction][query] = fn;
}
...
g.swipeend = function(e) {
  ...
  if(Math.abs(dy)>Math.abs(dx)) {
    if(dy>0) g.run_queries(g.target, g_swipe_queries.up);
    if(dy<0) g.run_queries(g.target, g_swipe_queries.down);
  } else {
    if(dx>0) g.run_queries(g.target, g_swipe_queries.right);
    if(dx<0) g.run_queries(g.target, g_swipe_queries.left);
  }
  ...
  for(let i in g_click_queries) if(isNaN(i)){
    ...
    for (xi of x) {
      if(e.target.isSameNode(xi)) {
        e.target.fn = g_click_queries[i].bind(e.target);
        e.target.fn()
      } else {
        el= e.target;
        while(el=el.parentNode) {
...
document.addEventListener('mousedown', g.swipestart, false);
document.addEventListener('touchstart', g.swipestart, false);
document.addEventListener('mouseup', g.swipeend, false);
...
g.post = function(path,params,fn){
  if(typeof params==Object) params=encodeURI(params)
...
g.dialog = function (p){
  ...
  if(p.type=='modal'){
    dsbg = '<div style="position:absolute;left:0;top:0;right:0;bottom:0" onclick="g(this.parentNode).remove()"></div>';
    g(document.body).append('<div class=\'gila-darkscreen\'>'+dsbg+append+'</div>');
...
g.loader = function (set=true) {
  if(set==true) {
    g(document.body).append('<div class=\'gila-darkscreen gila-loader\'><img src="src/core/assets/ajax-loader.gif" class="centered"></div>');
  } else {
