...
g.dialog = function(p){
  ...
  if(p.type=='modal'){
    closebtn = '<span class="closebtn" onclick="g.closeModal()">×</span>';
  ...
  if(p.type=='modal'){
    dsbg = '<div style="position:absolute;left:0;top:0;right:0;bottom:0" onclick="g.closeModal()"></div>';
...
g.closeModal = function() {
  g('.gila-popup').last().parentNode.remove();
  console.log(g('.gila-popup').all.length);
  if(g('.gila-popup').all.length===0) {
