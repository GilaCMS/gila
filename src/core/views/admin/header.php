<!DOCTYPE html>
<html lang="<?=Gila\Gila::config('language')?>">

<head>
  <base href="<?=Gila\Gila::base_url()?>">
  <title><?=((Gila\Gila::config('title')??'Gila CMS').' - '.($page_title??__('Administration')))?></title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width initial-scale=1">
  <link rel="icon" type="image/png" href="<?=Gila\Gila::config('admin_logo')?:'assets/gila-logo.png'?>">
  ...
  <div id="wrapper"<?=($_COOKIE['sidebar_toggled']=='true'? ' class="toggled"': '')?>>
    ...
    <div id="sidebar-wrapper">
      <div style="position: relative;height: 100px;">
        <a href="admin">
          <img style="max-width:180px;max-height:60px" src="<?=Gila\Gila::config('admin_logo')?:'assets/gila-logo.png'?>" class="centered">
        </a>
      </div>
      <ul class="g-nav vertical lazy" data-load="lzld/amenu?base=<?=Gila\Gila::url('#')?>">
      ...
      </ul>
    </div>
    <!-- /#sidebar-wrapper -->
    ...
    <div id="top-wrapper" class="g-group fullwidth bordered" style="vertical-align:baseline; background:white;">
      ...
      <?php if ('admin'!=Gila\Gila::config('default-controller')) {?>
      &nbsp;<a href="<?=Gila\Gila::base_url()?>" class="g-icon-btn g-group-item" title="Homepage" target="_blank"><i class='fa fa-home'></i></a>
      <?php } ?>

      <span class="g-group-item fullwidth text-align-right" id="topbar">
        <ul class="g-nav g-navbar" style="background:unset">
          <?php
          foreach (Gila\Gila::getList('badge') as $b) {
