...
gItem.prototype.attr = function (attr,val){
    if (typeof val === 'undefined') return this.all[0][attr]
    for(let value of this.all){
...
document.onclick = function(){
    for(let i in g_click_queries) if(isNaN(i)){
        ...
        for (xi = 0; xi < x.length; xi++) {
            if(event.target.isSameNode(x[xi])) g_click_queries[i]();
        }

    }
};

gItem.prototype.event = function (query,fn){
...
g.ajax = function(path,params,fn){
    var xhttp = new XMLHttpRequest();
    ...
    if(typeof params !== 'function') {
        ...
        xhttp.callback = function() {
          if (this.readyState == 4 && this.status == 200) fn(this.responseText); else fn(false);
        };
        ...
        xhttp.onload = function() {
          if (this.readyState == 4 && this.status == 200) params(this.responseText); else params(false);
        };
        xhttp.open("GET", path, true);
        xhttp.send();
    }
...
g.dialog = function (p){
    var default_params = {class:'',escape:true,body:'',title:'',foot:'',img:'',buttons:'ok'};
    for(i in default_params) if(typeof p[i] == 'undefined') p[i] = default_params[i];
    x = document.getElementById('gila-darkscreen')
    if (x) document.removeChild(x)
    ...
    if(p.buttons!='') for(btni of p.buttons.split(' ')){
        ...
        buttons+='<button class="btn '+btn.class+'" onclick="g.dialog.buttons.'+btni+'.fn()">'+btn.title+'</button>';
    }
    ...
    g(document.body).append('<div id=\'gila-darkscreen\''+dsclick+'"><div id=\'gila-popup\' class="'+p.class+'" onclick="event.stopPropagation()">'+closebtn+p.title+p.img+p.body+p.foot+'</div></div>');
}

g.dialog.buttons = [];
g.dialog.buttons.ok = {title:'Ok',fn:function(){
...
g.alert = function (html,type,callback){
    ...
    g.dialog({body:icon+'<h2>'+html+'</h2>',class:'small text-align-center',escape:false,buttons:buttons,foot:foot});
}
