...
if (typeof pnkPath == "undefined") pnkPath = "pnk/";
...
function pnk_create(div, tableid) {
	...
	if (typeof pnk_src[pnksrc] != 'undefined') {
		...
		$.getJSON( pnkPath + "pnk.php?t="+div.attr('pnk-src')+"&action=fields", function( data ) {
			pnk_src[pnksrc] = data;
			pnk_create_table(div, tableid, data);
		});
	}
...
//table table-stripped table-bordered table-hover
	div.html( '<div class="pnk-table-list">'+gsr+'<div><table class="table table-hover table-striped table-bordered table-condensed"'+tstyle+' id="'+tableid+'" pnk-src="'+pnksrc+'" cur-page="1" filters="&'+div.attr('filters')+'" group-by="'+groupby+'"><thead></thead><tbody></tbody><tfoot></tfoot></table></div></div><div class="pnk-edit" style="display:none"></div>');
//table-striped
	if(typeof data['search-boxes'] != 'undefined') for(i=0;i<data['search-boxes'].length;i++){
		var fid=data['search-boxes'][i];
		var df=data['fields'][fid];
...
function pnk_load(id)
{
	...
	$.getJSON( pnkPath + "pnk.php?t="+div.attr('pnk-src')+"&action=list&page="+page+porderby+pgroupby+pgroupfor+filters, function( data ) {
  ...
  if(typeof args.group_level == 'undefined') group_level = 0; else group_level = args.group_level+1;
    ...
    if(typeof pnkt['commands']!="undefined"){
      ...
      if(typeof row_id!="undefined") if(row_id!="") $.each( pnkt['commands'], function( key, key_value ) {
        ...
        var fa_icons={'edit':'pencil','delete':'trash-o','pdf':'file','clone':'copy','list':'chevron-right'};

        $.each(fa_icons, function( index, value ){
          if(key==index){
            if(pnkt['td-command']!=key) ctd=ctd+'<i class="fa fa-'+value+' pnk-com com-'+index+'"></i>';
              else ctd=ctd+'<i style="display:none" class="fa fa-'+value+' pnk-com com-'+index+'"></i>';
          }
          if(key_value==index){
            if(pnkt['td-command']!=key) ctd=ctd+'<i class="fa fa-'+value+' pnk-com com-'+index+'"></i>';
              else ctd=ctd+'<i style="display:none" class="fa fa-'+value+' pnk-com com-'+index+'"></i>';
...
function pnk_load_thead(id,data)
{
  ...
  if(typeof pnk_srcid['bulk_actions']!="undefined"){
		gth=gth+'<th style="width:28px"></th>'; // Leave a column for the checkboxes
	}//<input type="checkbox">
    ...
    //var title;
    if(df.title) title = df.title; else title = field;
		//title='<p>'+title+'</p>';
...
$(document).on('click','.pnk-table table tr td p .pnk-roles',function()
{
	...
	if(df['type']=='roles'){
		$.ajax({ url: pnkPath + "pnk.php?action=keys", dataType: "json",type: 'post',
		...
		dialog_buttons[_e('Update')]=function() {
			...
			$.ajax({ url: pnkPath + "pnk.php?t="+table_src+"&action=keys&update", dataType: "json",type: 'post',
...
function pnk_delete_row(tr) {
  ...
  $.ajax({
		url: pnkPath + "pnk.php?t="+t+"&action=delete",
		data: { 'id' : id },
		type: 'post',
		success: function(data) {
...
/*
$(document).on('click','.pnk-table .com-delete', function(){
	if ( confirm(_e("Delete registry?")) == false) return;
  pnk_delete_row($(this).closest('tr'));
});

$(document).on('click','.pnk-table .com-pdf',function(){
	var t=$(this).closest('.pnk-table').attr('pnk-src');
	var tr=$(this).closest('tr');
	var id=tr.attr('row-id');
	window.open(pnkPath + "pdf.php?t="+t+"&id="+id);
});*/
...
/*
$(document).on('click','.pnk-table .com-edit',function()
{
	var tr=$(this).closest('tr');
	var id=tr.attr('row-id');
	var tid=tr.closest('table').attr('id');
	edit_dialog(tid,"edit",id);
});
$(document).on('click','.pnk-table .com-clone',function()
{
	var tr=$(this).closest('tr');
	var id=tr.attr('row-id');
	var tid=tr.closest('table').attr('id');
	edit_dialog(tid,"clone",id);
});
*/
...
function edit_dialog(tid,action,id=""){
  ...
  if(action=="clone"){
	...
	$.ajax({
		url: pnkPath + "pnk.php?t="+t+"&action=edit", data: { 'id' : id }, dataType: "json",type: 'post',
		success: function(data) {
    ...
    /*  for(i=0; i<pnk_src[t]['joins'].length; i++){ //pnk_src[t]['joins'].length
        title = pnk_src[t]['joins'][i].title;
        field_o = "<option value='1'>One</option><option value='2'>Two</option><option value='3'>three</option>";
        sopt=sopt+'<div class="update-div" jt="'+i+'"><label for="'+i+'">'+title+'</label><select multiple>'+field_o+'</select></div>';
...
/*
      ...
      $(".pnk-select").chosen({disable_search_threshold: 10});
      //if($.select2)
      // $(".pnk-select").select2();
			if(typeof($.datetimepicker) == "function") $(".datetimepicker").datetimepicker({format: 'Y-m-d H:i:s'});
...
$('body').on('click','.pnk-edit-save',function(e){
	...
	data =jsonEscape(data);
		...
		$.ajax({
			url: pnkPath + "pnk.php?t="+t+"&action=update",
			data: { 'erows' : data },
			cache:  false ,
			dataType: "json",
			type: 'post',
...
function createInput(df,v){
  ...
  if(df['type']=='joins'){
	...
	// Textarea
	if(df['type']=='text'){
		return '<textarea class="pnk-input">'+v+'</textarea>';
	}
	...
	// Date
	if(df['type']=='date'){
		return '<input class="pnk-input datepicker" value="'+v+'">';
	}
	// Datetime
	if(df['type']=='datetime'){
		return '<input class="pnk-input datetimepicker" value="'+v+'">';
	}
...
$(document).on('click','.pnk-com',function(){
  ...
  if($(this).hasClass("com-refresh")) command="refresh";
  if($(this).hasClass("com-edit")) command="edit";
  if($(this).hasClass("com-delete")) command="delete";
  if($(this).hasClass("com-clone")) command="clone";
  e.src = $(this).closest('table').attr('pnk-src');
  e.table_id = $(this).closest('table').attr('id');
  e.row_id = $(this).closest('tr').attr('row_id');
...
Pnk.commands.clone = { fa: "pencil", title: "Clone", fn: function(e){ edit_dialog(e.table_id,"clone",e.row_id); }};
Pnk.commands.pdf = { fa: "file", title: "Pdf", fn: function(e){ window.open(pnkPath + "pdf.php?t="+e.src+"&id="+e.row_id); }};
Pnk.commands.delete = { fa: "delete", title: "Delete", fn: function(e){
...
function pnk_window_open(e,file) {
  var id = e.table_id;//$(this).closest('.pnk-tools').attr('table-id');
	...
	window.open(pnkPath + "pnk.php?t="+t+"&action=pnkfile&file="+file+"&orderby="+orderby+"&groupby="+groupby+filters);
}
...
$(document).on("click",".col-group",function(){
  ...
  // Get the rows from the source file
	$.getJSON( pnkPath + "pnk.php?t="+t+"&action=list"+groupby+"&page=1"+filters, function( data ) {
...
$(document).on("click",".col-group-open",function(){
  ...
  //$(this).parent().attr("group_level");
});

$(document).on("change",".pnk-table td input[type='checkbox']",function(){
  if(this.checked){
    $(this).closest('tr').addClass("selected");
    //$(this).closest('table').find('th input[type="checkbox"]').prop( "checked", true );
  }else{
